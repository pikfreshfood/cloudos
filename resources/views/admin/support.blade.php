            ...
            <div style="display: flex; flex-direction: column; gap: 8px;">
                @forelse ($conversations as $conv)
                    <a href="{{ route('admin.support', ['user_id' => $conv['user']->id]) }}" 
                       style="padding: 12px; border-radius: 8px; text-decoration: none; {{ ($selectedUser && $selectedUser->id === $conv['user']->id) ? 'background: var(--line);' : 'background: transparent;' }}"
                    >
    ...
    <div style="flex: 1;">
        <div class="card">
            ...
            @else
                ...
                <div class="muted" style="padding: 48px; text-align: center;">
                    Select a user from the left sidebar to view and reply to their messages.
                </div>
            @endif
        </div>
    </div>
